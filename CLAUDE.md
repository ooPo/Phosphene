...
**Display** (`src/display.cpp`): each `present()` allocates a `SDL_GPUTransferBuffer`, copies the framebuffer in, does a copy pass into the persistent GPU texture, then blits to the swapchain with nearest-neighbour filtering.
